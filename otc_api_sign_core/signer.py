# -*- coding:utf-8 -*-
#
# Copyright (c) OpenTelekomCloud Community. 2025. All rights reserved.
#
import copy
import sys
import hashlib
...
if sys.version_info.major < 3:
    ...
    def hmacsha256(byte, msg):
        return hmac.new(
            byte.encode("utf-8"), msg.encode("utf-8"), digestmod=hashlib.sha256
        ).digest()
...
HContentSha256_UNSIGNEDPAYLOAD = "UNSIGNED-PAYLOAD"
HXSecurityToken = "X-Security-Token"
...
def CanonicalRequest(req, sHeaders):
    ...
    if hencode is None or hencode != HContentSha256_UNSIGNEDPAYLOAD:
...
class Signer:
    # Initialize a new Signer object with empty Key and Secret.
    # The Key and Secret properties must be set before calling Sign or Verify.
    # Key is the Access Key (AK) and Secret is the Secret Key (SK).
    # If SecurityToken is set, it will be added to the request header as X-Security-Token
    # for temporary credentials (in this case ak is SecurityAccessKey and sk is SecuritySecretKey).
    def __init__(self):
        self.Key = ""
        self.Secret = ""
        self.SecurityToken = ""
        
    # Verify checks that the provided authorization value matches the
    # signature computed from the request and the Signer's Secret.
    # It returns true if the signature matches, false otherwise. 
    ...
    def Sign(self, req):
        ...
        # if security token is set, add it to header
        if self.SecurityToken != "":
            req.headers[HXSecurityToken] = self.SecurityToken
